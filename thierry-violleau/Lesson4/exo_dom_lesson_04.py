...
# https://www.leboncoin.fr/voitures/offres/<region>/?th=1&q=Renault%20Zo%E9&parrot=0
# https://www.leboncoin.fr/voitures/offres/ile_de_france/?th=1&q=Renault%20Zo%E9&parrot=0
# https://www.leboncoin.fr/voitures/offres/provence_alpes_cote_d_azur/?th=1&q=Renault%20Zo%E9&parrot=0
# https://www.leboncoin.fr/voitures/offres/aquitaine/?q=Renault%20Zo%E9
...
def get_car_ads_for_regions(regions):
    ...
    for region in regions:
        for url, pro in get_car_ads_for_region(region, car_name).items():
            ...
            parser = BeautifulSoup(data.text, 'html.parser')
            ad = { 'url': url, 'pro': pro }
            for value_node in parser.findAll(class_=["value", "clearfix"]):
                try:
    ...
    df = df.drop(['description', 'all_text', 'url'], axis=1)
    return df
