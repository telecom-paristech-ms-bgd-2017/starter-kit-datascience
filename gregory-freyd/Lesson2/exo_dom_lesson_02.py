...
def getFiguresForCityWithURL(url):
    """
    Returns all the figures for a city given an URL
    """
    figures_city = {}

    figures_city_page = requests.get(url)
    soup_city = BeautifulSoup(figures_city_page.text, 'html.parser')

    html_table_rows = soup_city.find_all('tr')

    # Looping in HTML table rows to get the requested info, then on columns
...
def getFiguresForCity(year_min, year_max, id_commmune, id_departement):
    ...
    for year in range(year_min, year_max + 1):
        ...
        figures_city = getFiguresForCityWithURL(
